...
export default function JobSeekerPersonalityPage() {
  ...
  return <div className="personality-page">
    ...
    {step === "result" && <main className={`personality-result personality-result-${result.color}`}>
      ...
      <section className="personality-path-gateway"><div className="personality-path-gateway__preview" aria-hidden="true"><span /><span /><span /><i /><i /></div><div><span className="personality-card-label">从人格到行动</span><h2>认识了自己的求职人格，接下来看看你的 Path</h2><p>根据你的准备情况，生成一条可以随时调整的求职路线。</p></div><button type="button" onClick={() => navigate("/note/fall-recruiting/autumn-recruitment-roadmap.md?pathSetup=1")}>生成我的求职 Path <ArrowRightOutlined /></button></section>
