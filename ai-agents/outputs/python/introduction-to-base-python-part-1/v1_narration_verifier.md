...
[PAUSE_SHORT]
These programs will do things like arithmetic calculations, showing output, and taking input from a user.
[PAUSE_SHORT]
You'll also recognize Python's built-in data types.
[PAUSE_SHORT]
And you'll choose the right structures for basic data storage.
[PAUSE_SHORT]
...
[PAUSE_SHORT]
Python is known for its clear, readable code.
[PAUSE_SHORT]
...
[PAUSE_SHORT]
By the end of this lesson, you'll be able to write simple programs.
[PAUSE_SHORT]
These programs will perform calculations, interact with the user, and store various types of data.
[PAUSE_SHORT]
Let's dive in!
...
Python isn't just for displaying text.
[PAUSE_SHORT]
...
*   `-` for subtraction
*   `*` for multiplication
*   `/` for division. This always gives you a `float`, even if it's a whole number.
...
[PAUSE_SHORT]
Data that your program needs to use or manipulate.
[PAUSE_SHORT]
...
[PAUSE_SHORT]
They are one of the most frequently used data structures.

[PAUSE_SHORT]

2.  **Tuples (`tuple`):**
    *   An **ordered** collection of items, similar to lists.
    *   They are **immutable**. Once created, you cannot change, add, or remove items.
